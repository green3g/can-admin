    ...
    return Control.extend({
        ...
        getVersionSelectText: function (version) {
            return pageConfig.versionsSelectText 
				? pageConfig.versionsSelectText.replace(/<%=\s*version\s*%>/, String(version)) 
				: version;
        },
        getVersionedParentPath: function (version) {
			
            var path = (this.docConfig.versionDest || './<%= version %>/<%= name %>').replace(/<%=\s*version\s*%>/, String(version))
				.replace(/<%=\s*name\s*%>/, String(pageConfig.project.name));
            return dirname(path);
        },
        getDefaultParentPath: function () {
            var path = (this.docConfig.defaultDest || './<%= name %>').replace(/<%=\s*name\s*%>/, String(pageConfig.project.name));
            return dirname(path);
        },
