...
export function mapFormatter (coords) {
    const projected = ol.proj.transform(coords,
    'EPSG:4326', 'EPSG:3857');
    const props = {
        longitude: coords[0],
        latitude: coords[1],
    ...
    return stache('<ol-map x="{{longitude}}" y="{{latitude}}" zoom="18" {map-options}="mapOptions" />')(props);
}
